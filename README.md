...
( \
   cd /usr/local/bin \
&& curl https://raw.githubusercontent.com/hugojosefson/isolate-in-docker/deno/isolate-in-docker.ts -o isolate-in-docker.ts \
&& chmod +x isolate-in-docker.ts \
&& ln -s isolate-in-docker.ts isolate-in-docker \
